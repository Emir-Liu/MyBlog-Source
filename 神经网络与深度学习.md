...
# 2.反向传播算法的工作原理
之前，我们已经通过神经网络，使用梯度下降来学习其权重和偏置。但是，其中我们没有详细讲解如何计算成本函数的梯度。这里我们将详细介绍反向传播算法。
...
上面，我们使用了一个δ binom{l}{j} ,用来表示第l层中的第j个节点的神经元的误差。
...
首先我们回忆一下，z为神经元的加权输入，a为神经元的加权输出。
那么第一个方程就是δ的定义，然后使用链式法制推导出来的。
